...
source side); both are posted rather than silently dropped.
...
def select_from_article(offers: list[ET.Element], start_article: str) -> list[tuple[str, ET.Element]]:
    ...
    numbered = []
    for offer in offers:
        article = offer.findtext("article") or ""
...
async def _post_offer(bot: Bot | None, article: str, offer: ET.Element, dry_run: bool) -> None:
    ...
    async def _send() -> None:
        if not pictures:
            ...
            await bot.send_photo(chat_id=config.SALES_GROUP_CHAT_ID, photo=pictures[0], caption=caption)
