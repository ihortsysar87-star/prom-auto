...
8. "description"/"description_ru" мають бути ПОВНИМ описом товару, а не скороченим переказом: перенеси з наданого тексту сторінки ВСІ деталі, характеристики, переваги, комплектацію, умови використання тощо, які там є - нічого не пропускай і не узагальнюй заради стислості. Це переклад/перенесення повного змісту, а не короткий summary. Дозволено лише прибрати технічний "шум" сторінки (меню, хлібні крихти, кнопки "додати в кошик" тощо) - весь текст, що реально описує сам товар, має залишитися.
...
def extract_product_from_page(url: str, json_ld: dict | None, page_text: str) -> dict:
    ...
    if json_ld:
        parts.append(
            ...
            + json.dumps(json_ld, ensure_ascii=False, indent=2)[:20000]
        )
